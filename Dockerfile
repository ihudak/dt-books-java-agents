...
RUN  sed -i s/tenant_id/$TENANT_ID/g /var/lib/dynatrace/oneagent/agent/conf/standalone.conf && \
     sed -i s/tenant_token/$OA_TOKEN/g /var/lib/dynatrace/oneagent/agent/conf/standalone.conf && \
     sed -i "s|tenant_url|$TENANT_URL|g" /var/lib/dynatrace/oneagent/agent/conf/standalone.conf
